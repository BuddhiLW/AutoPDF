...
func NewPersistentService() *PersistentService {
	...
	configDir := filepath.Join(homeDir, ".autopdf")
	...
	if err := os.MkdirAll(configDir, 0755); err != nil {
		...
		configFile = filepath.Join(configDir, "autopdf-config.yaml")
	}
	...
	if verboseLevel := persister.Get("verbose_level"); verboseLevel != "" {
		if level, err := strconv.Atoi(verboseLevel); err == nil && level >= 0 && level <= 4 {
			config.VerboseLevel = adapters.LogLevel(level)
		}
	}
...
func (ps *PersistentService) SetVerboseLevel(level adapters.LogLevel) error {
...
func (ps *PersistentService) GetVerboseLevel() adapters.LogLevel {
