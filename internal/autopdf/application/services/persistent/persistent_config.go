...
type PersistentConfig struct {
	// Verbose settings
	VerboseLevel   logger.LogLevel `yaml:"verbose_level" json:"verbose_level"`
...
func DefaultPersistentConfig() *PersistentConfig {
	return &PersistentConfig{
		VerboseLevel:   logger.Detailed, // Default to detailed logging
		VerboseEnabled: true,
...
func (pc *PersistentConfig) SetVerboseLevel(level logger.LogLevel) {
	pc.VerboseLevel = level
	pc.VerboseEnabled = level > logger.Silent
...
func (pc *PersistentConfig) GetVerboseDescription() string {
	descriptions := map[logger.LogLevel]string{
		logger.Silent:   configs.VerboseLevelDescriptions[0],
		logger.Basic:    configs.VerboseLevelDescriptions[1],
		logger.Detailed: configs.VerboseLevelDescriptions[2],
		logger.Debug:    configs.VerboseLevelDescriptions[3],
		logger.Maximum:  configs.VerboseLevelDescriptions[4],
	}
