...
func (iaa *InternalApplicationAdapter) GeneratePDF(cfg *config.Config, template config.Template) ([]byte, map[string]string, error) {
	// Merge configuration
...
func (iaa *InternalApplicationAdapter) createDocumentService(cfg *config.Config) *documentService.DocumentService {
	...
	latexAdapter := latex.NewLaTeXCompilerAdapter(cfg)
